...
class DOM {
  ...
  static css(sel, model) {
    ...
    let cls = sel.split('_');
    sel = cls.shift();
    if (sel === 'h' || sel.endsWith(' h') || sel.endsWith('.h')) {
      cls = cls.length ? ('.' + cls.join('.')) : '';
      sel = Array(6).fill(sel).map((s, i) => s + (i + 1) + cls).join(', ');
      cls = [];
    }
